...
| `UtilitiesSection` | inline | Sorted to surface utilities serving the most people that still have an unresolved health-based violation. Compliant systems fall to the bottom. Each links to `/state/[state]/water/[slug]`. Inline ownership chip per row (Municipal / Private / Tribal / State-owned / Federal / Mixed) sourced from SDWIS `owner_type_code`. |
...
| `UtilitiesSection` | inline | Public water utilities serving the county (when SDWIS county join has resolved). Links to `/state/[state]/water/[slug]`. Ownership chip per row (Municipal / Private / Tribal / State-owned / Federal / Mixed). |
...
| `WaterSection` | inline | Compliance-posture lede + 4-stat strip + utilities-serving table linking each row to `/state/[state]/water/[slug]`. Ownership chip per row (Municipal / Private / Tribal / State-owned / Federal / Mixed) helps readers distinguish municipal utilities from mobile-home-park PWSes serving the same city. Footer note: PWS is the regulated entity, not the city. |
...
| Section | Component | What it shows |
|---|---|---|
| `WaterHero` | inline + `HeroChart` | Eyebrow, H1 (`{city} Water Quality` derived from SDWIS `city_name`), lede with PWS name + PWSID + population served + 5-year violation tally. Meta line under the H1 carries `PWSID · {source-water type} · {ownership chip}` (Municipal / Private / Tribal / State-owned / Federal / Mixed). Aside: 5-year stat strip. Hero chart is annual violation count history. CTA to EPA SDWIS deep-link. |
...
| `WaterLinkageSection` | inline | **Differentiator.** SDWIS PWSes drawing groundwater within N miles of the site (N=3 for v1, tunable). Per-PWS row: utility name + ownership chip (Municipal / Private / Tribal / State-owned / Federal / Mixed), distance, source-water type, treatment notes, most recent health-based violation if any, link to `/state/[state]/water/[slug]`. Renders "no groundwater PWSes within {N} mi" empty-state when none match — empty result is itself a finding. |
| `EquitySection` | inline + `EquityStub` | Four-tier preference: 1-mile block-group buffer (population-weighted across TIGER 2020 BG centroids inside the radius — standard EPA EJScreen "Define an Area" approach) → host city → containing county → state. Buffer-anchored tier shows ACS demographics only; national-percentile and EJ-disparity layers attach when fallback drops to host city or county. NPL proximity itself contributes to the national EJ pattern, so this section anchors why the page exists. |
